...
<error type="unauthorized">unauthorized use of xml interface</error>}
  ...
  def make_uri_from_arguments
    ...
    # p=38b99ce9ec87&a=Kevin+Bacon&b=Ian+McKellen
    @uri = "http://oracleofbacon.org/cgi-bin/xml?enc=utf-8&p="
    @uri+= CGI.escape(@api_key) 
    @uri +="&a=" 
    @uri += CGI.escape(@to)
    @uri +="&b=" 
    @uri += CGI.escape(@from)
  ...
  class Response
    ...
    def parse_error_response
     ...
     e = @doc.xpath('//*[@type]')[0].attr('type')
     if e == "badinput" then
       
       @type = :badinput
       
     elsif e == "unlinkable" then
       @type = :unlinkable
     elsif e == "unauthorized" then
       @type = :unauthorized
     else
    ...
    def parse_graph_response
      #Your code here
      @type = :graph
      resp_Array = @doc.xpath('//actor').map(&:text).zip(@doc.xpath('//movie').map(&:text))
      # resp_Array.flatten
      # resp_Array.compact
      @data = resp_Array.flatten
      @data = @data.compact
    end
